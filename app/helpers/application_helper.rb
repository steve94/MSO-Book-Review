module ApplicationHelper
  ...
  def auto_complete_input(object, method, title, path) # This are for required fields
    tag_options = {:class => "string auto_complete required", :size => 50}
    completion_options = {:url => path, :method => :get, :with => "'search=' + element.value", :skip_style => true}
    
    output = "<div class=\"input string  required\">\n"
    output << "<label class=\"string auto_complete required\">* #{title}</label>"
    output << text_field_with_auto_complete(object, method, tag_options, completion_options)
    output << "</div>"
